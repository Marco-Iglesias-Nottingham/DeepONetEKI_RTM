function [opt, pressures, flow_rates, filling_factors] = cvfem2d_obsrv(opt,obsrv_times,gamma,beta,frac)
...
param(1)=opt.bndry.pinlet;
param(2)=frac;
param(3)=gamma;
param(4)=beta;


opt.cvfem.u(opt.bndry.inlet_flag==1) = gD(opt.bndry.inlet_pos,param,0);
...
while ~isFilled(opt.cvfem.fFactor,opt.mesh.nnode,opt.bndry.vent_idx)
    [opt.cvfem, opt.bndry] = still_solver(opt.cvfem,opt.mesh,opt.bndry,param,opt.cvfem.fTime);
...
if tindex <= length(obsrv_times)
    if opt.cvfem.fTime < obsrv_times(tindex)
        for i=tindex:length(obsrv_times)
            pressures(:,i)=opt.cvfem.u;
            filling_factors(:,i)=ones(opt.mesh.nnode,1);
        end
    end
end
